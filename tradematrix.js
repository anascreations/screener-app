...
// Standard UI updates
buildDialChart('ma-dial-canvas',adjScore);
const pieData=[
  {label:'MA Stack',val:(f1_pass?20:0)+(f2_pass?18:0)+(f3_pass?14:0)+(f4_ma200?8:0),max:60,c:'var(--accent)'},
  {label:'KDJ/MACD', val:(kdj?.pass===true?16:0)+(macd?.pass===true?14:0),max:30,c:'var(--green)'},
  {label:'ADX/DMI',  val:(adxS?.pass===true?16:0)+(dmiS?.pass===true?12:0),max:28,c:'var(--yellow)'},
  {label:'Vol/ST/RSI',val:(volS?.pass===true?10:0)+(stS?.pass===true?6:0)+(rsiS?.pass===true?10:0),max:26,c:'var(--orange)'},
];
buildPieChart('ma-pie','ma-pie-legend',pieData);
const checkRows=[
...
buildSignalMeter('ma-signal-meter',checkRows.split('check-pass').length-1,checkRows.split('check-fail').length-1);
const alignRows=[
...
<div class="qpp-split-fill">
            ...
            <div class="qpp-split-part">
              <div class="qpp-split-label">${ma5 && ma5 < price ? "QUEUE AT MA5" : "QUEUE AT MA5 (est.)"}</div>
              <div class="qpp-split-price" style="color:var(--green)">${currency}${(ma5 && ma5 < price ? ma5 : price * 0.972).toFixed(dp)}</div>
            ...
            <div class="qpp-split-avg">
              ...
              <span class="qpp-split-avg-val" style="color:var(--accent)">${currency}${((price + (ma5 && ma5 < price ? ma5 : price * 0.972)) / 2).toFixed(dp)}</span>
            </div>
